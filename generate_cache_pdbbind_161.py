...
def mol2_to_pdb(mol2_file, pdb_file, timeout=30):
    ...
    Suppress warnings about missing element symbols (columns 77-78).
    """
    cmd = ["obabel", "-imol2", mol2_file, "-opdb", "-O", pdb_file]
    try:
        # Capture stderr to suppress OpenBabel warnings about element symbols
...
def prepare_protein_pocket(protein_pdb, ligand_pdb, pocket_pdb, selected_pdb):
    ...
    if not os.path.exists(protein_pdb):
        raise RuntimeError(f"Protein PDB not found: {protein_pdb}")
    if not os.path.exists(ligand_pdb):
        raise RuntimeError(f"Ligand PDB not found: {ligand_pdb}")
    
    # Check if files are not empty
    if os.path.getsize(protein_pdb) == 0:
        raise RuntimeError(f"Protein PDB is empty: {protein_pdb}")
    if os.path.getsize(ligand_pdb) == 0:
        raise RuntimeError(f"Ligand PDB is empty: {ligand_pdb}")
    
    # 1. Protein sadece protein atomlarını içeriyor olmalı
    try:
        structure = pr.parsePDB(protein_pdb)
    except Exception as e:
        raise RuntimeError(f"Failed to parse protein PDB {protein_pdb}: {e}")
    ...
    if structure is None or structure.numAtoms() == 0:
        raise RuntimeError(f"No protein atoms found in: {protein_pdb}")
...
def run_apbs(protein_pdb, protein_pqr, cache_dir, case_name, box_size, grid_step=1.0, timeout=60):
    ...
    CRITICAL FIX: For consistency with atomic features:
    - APBS always computes at 161x161x161 grid (spacing=1.0 Å)
    - Then resizes to target box_size if different
    - This ensures grid_center and boundaries match tfbio.Featurizer
    """
    ...
    # FIXED: Always compute APBS at 161x161x161 with spacing=1.0 Å
    apbs_box_size = 161
    apbs_grid_step = 1.0
    grid_shape, grid_min, grid_max, grid_center = get_grid_params(protein_pdb, apbs_box_size, apbs_grid_step)
    # FIXED: glen = (dime - 1) * spacing to match tfbio's max_dist calculation
    # tfbio uses max_dist = (box_size - 1) / 2.0 = 80.0 Å, so total span = 160.0 Å
    cglen = [160.0, 160.0, 160.0]  # Total grid length in Angstroms (NOT 161!)
    dime = [apbs_box_size] * 3  # 161x161x161 grid points
    grid_spacing = cglen[0] / (dime[0] - 1)  # = 160.0 / 160 = 1.0 Å exactly
...
""")
    ...
    # FIXED: Always call resize_grid to convert 1D to 3D and resize if needed
    # resize_grid handles both reshaping from 1D and scaling to target_shape
    apbs_grid_resized = resize_grid(apbs_grid, (box_size, box_size, box_size))
    
    # FIXED: Return grid_center and parameters for the target box_size
...
def get_dataset_paths(dataset="pdbbind", box_size=72):
    ...
    if dataset == "pdbbind":
        pdbbind_root = "/Users/tevfik/Sandbox/github/PHD/data/pdbbind/refined-set-only-used-in-codon-tests/"
        output_dir = f"/Users/tevfik/Sandbox/github/PHD/data/pdbbind/refined-set-only-used-in-codon-tests/box{box_size}/"
    elif dataset == "scpdb":
...
if __name__ == "__main__":
    ...
    dataset_fits_file = "minimal-set.txt"
