...
class Featurizer():
    ...
    def featurize(self, pdb_file, box_size, resolution, grid_center):
        """Load PDB, get features, and optionally convert them into a 3D grid."""
        mol = next(pybel.readfile('pdb', pdb_file))
        coords, features = self.get_features(mol, molcode=-1.0)
        coords -= np.asarray(grid_center, dtype=float)
        max_dist = (box_size - 1)/ 2.0
        grid = make_grid(coords, features, max_dist, grid_resolution=resolution)
        return grid
...
def make_grid(coords, features, max_dist, grid_resolution=1.0):
    ...
    # # 0 olmayan grid noktalarının koordinatlarını bul:
    # nonzero_coords = np.argwhere(np.any(grid != 0, axis=0))  # (N, 3) shape; axis=0: herhangi bir feature sıfırdan farklı mı?

    # print("0 olmayan grid noktaları ve feature değerleri:")
    # for coord in nonzero_coords:
    #     # Feature boyutu ilk axis olduğu için, bu noktadaki tüm feature değerlerini yazdırabilirsin:
    #     values = grid[:, coord[0], coord[1], coord[2]]
    #     print(f"Coord: {tuple(coord)}, Features: {values}")
